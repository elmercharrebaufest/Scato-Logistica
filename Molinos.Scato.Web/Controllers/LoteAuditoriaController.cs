...
{
    [Autorizacion(PermisosScato.ArmarLoteAuditoria)]
    public class LoteAuditoriaController : BaseController
    {
        ...
        public LoteAuditoriaController(ILogger log, IServicioRepositorio servicio, IServicioComandos serviciosComandos, IFirmaProvider configuracion, IFirmaProvider
             firma)
            : base(servicio)
        {
            this.log = log;
        ...
        [DatosUsuario]
        public ActionResult BuscarLote(DatosUsuario datosUsuario, BuscarLoteAuditoriaDto filtro, int pagina = 1, string ordenarPor = "Fecha", DirOrden dirOrden = DirOrden.Desc)
        {
        ...
        [DatosUsuario]
        public ActionResult Listar(DatosUsuario datosUsuario, BuscarLoteAuditoriaDto filtro, int pagina = 1, string ordenarPor = "Fecha", DirOrden dirOrden = DirOrden.Desc)
        {
            filtro.CentroId = datosUsuario.CentroId;
            ListarConsulta(filtro, pagina, ordenarPor, dirOrden);
            return View("Listar");
        ...
        private void ListarConsulta(BuscarLoteAuditoriaDto filtro, int pagina, string ordenarPor, DirOrden dirOrden)
        {
            ...
            ViewBag.Items = servicio.ListarPaginadoAuditoriaLote(filtro, paginacion);
            var camaras =
        ...
        {
            ViewBag.Items = servicio.ListarMuestrasPorLoteAuditoria(id, new Paginacion(ordenarPor, dirOrden, pagina, 20));
            ViewBag.NumeroDeLote = servicio.ObtenerNumeroLoteAuditoria(id);
            ViewBag.LoteId = id;
            return View();
        }
        ...
        {
            var lote = servicio.ObtenerLoteAuditoriaParaImpresion(loteId);
        ...
        {
            var loteDto = servicio.ObtenerLoteAuditoriaParaArchivo(loteId);
            ...
            ListarConsulta(new BuscarLoteAuditoriaDto { NroLote = loteDto.NumeroDeLote }, 1, "Id", DirOrden.Asc);
            return View(vista);
        }
        
        public ActionResult DescargarBuenosAires(LoteAuditoriaDto loteDto, DatosUsuario datosUsuario)
        {
        ...
        [AllowAnonymous]
        public JsonResult EnvioDeLoteAutomatico(int centroId, int materialId, int camaraId, string mail)
        {
            ...
            if (materiaPorCentro == null)
            {
                result.Data = Textos.MaterialRequiereTecnologia;
            }
            else if (!servicio.ExistenMuestraEnvioACamaraAuditoriaPendientes(materialId, centroId))
            {
            ...
            else
            {
                ...
                var loteDto = new LoteAuditoriaDto
                {
                    Fecha = DateTime.Now,
                ...
                try
                {
                    var resultado = (ResultadoCrear)serviciosComandos.Ejecutar(new CrearLoteAuditoria { Dto = loteDto });
                    if (resultado != null && !resultado.HayErrores)
                    {
                        loteDto = servicio.ObtenerLoteAuditoriaParaArchivo(resultado.Id);
                        if (!string.IsNullOrEmpty(mail))
                        {
                            loteDto.CamaraEmail = mail;
                        }
                        if (loteDto.CamaraFormatoDeArchivo == CamaraFormatoDeArchivo.Rosario)
                        {
                ...
                {
                    log.Error(e, "Error al generar el lote Auditoria");
        ...
        public ActionResult DescargarRosarioZip(LoteAuditoriaDto loteDto, bool generarSoloArchivo1 = false)
        {
        ...
        public ActionResult DescargarBahiaBlanca(LoteAuditoriaDto loteDto)
        {
            string str01;
        ...
        public ActionResult SubirArchivo()
        {
            ...
            {
                log.Debug("Subiendo archivos de auditoria " + Request.Files.Count);
                for (int i = 0; i < Request.Files.Count; i++)
                {
                    var resultado = ProcesarArchivoResultado(Request.Files[i].InputStream);
            ...
            return RedirectToAction("BuscarLote");
        }

        private Resultado ProcesarArchivoResultado(Stream stream)
        {
            var resultado = new Resultado();
            try
            {
                log.Debug("ProcesarArchivoResultado");
                var resultadosCamara = CararArchivo(stream);
                log.Debug("SincronizarResultadosCamara" + resultadosCamara.Count());
                resultado = serviciosComandos.Ejecutar(new SincronizarResultadosCamara{ Resultados = resultadosCamara });
                log.Debug("SincronizarResultadosCamara fin");

            }
            catch (Exception ex)
            {
                log.Error(ex, "archivos de auditoria ");
                resultado.Errores.Add("ArchivoRutaDestino", ex.Message);
            }
            return resultado;
        }
        
        private static List<ResultadoCamaraDto> CararArchivo(Stream stream)
        {
            var document = new XmlDocument();
            document.Load(stream);
            XmlNodeList nl = document.SelectNodes("Solicitudes/Solicitud");
            var muestras = new List<ResultadoCamaraDto>();
            foreach (XmlNode solicitud in nl)
            {
                XmlNodeList ensayos = solicitud.SelectNodes("EnsayoTecnica");
                for (int i = 0; i < ensayos.Count; i++)
                {
                    var ensayo = ensayos[i];
                    //var esGrado = ensayo.SelectNodes("Ensayo").Item(0).Attributes["Descripcion"]?.Value;
                    var esGrado = ensayo.SelectSingleNode("Ensayo[@Descripcion='Grado']");
                    if (esGrado != null)
                    {
                        var valor = ensayo.SelectSingleNode("ResultadosYComponentes/ResultadoComponente/ResultadoDelEnsayo")?.Attributes["Alfabetico"]?.Value;
                        if (!string.IsNullOrEmpty(valor))
                        {
                            muestras.Add(new ResultadoCamaraDto
                            {
                                NumeroMuestra = solicitud.Attributes["NumeroMuestra"].Value,
        ...
        private void GenerarRosario(LoteAuditoriaDto loteDto, out string archivo01, out string archivo02, out string archivo03)
        {
        ...
        private string EnviarArchivoRosarioPorEmail(LoteAuditoriaDto loteDto)
        {
            var result = "El envio del lote fue Exitoso";

            try
            {
                if(loteDto.CamaraEmail == null)
                {
                    return "La camara no tiene mail configurado";
                }
                ...
                message.Subject = " Generación de archivos lotes internos para " + loteDto.CentroDesc + " - " + DateTime.Now.Formatted();
        ...
        private string EnviarArchivoBuenosAiresPorEmail(LoteAuditoriaDto loteDto)
        {
            var result = "El envio del lote fue Exitoso";
        ...
        private string EnviarArchivoBahiaBlancaPorEmail(LoteAuditoriaDto loteDto)
        {
            var result = "El envio del lote fue Exitoso";
        ...
        private Presentacion GenerarBuenosAires(LoteAuditoriaDto loteDto, CentroDto centro)
        {
        ...
        private void GenerarBahiaBlanca(LoteAuditoriaDto loteDto, out string archivo)
        {
            var stringBuilder01 = new StringBuilder();
