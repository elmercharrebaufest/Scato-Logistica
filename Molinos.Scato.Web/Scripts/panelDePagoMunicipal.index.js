$(document).ready(function () {
    ...
    $("#btnTransmitir").click(function () {
        if ($("#transmisiones").val().length != 0) {
            ...
            $.post($("#btnTransmitir").data().url, { registrosSeleccionados: t }, function (data) {
    ...
    //    return Globalize.parseDate($('.IngresoDesde input:first').val()) <= Globalize.parseDate(value) || $('.IngresoHasta input:first').val() == "" || $('.IngresoDesde input:first').val() == "";
    //}, $('.IngresoHasta').data().secondDateValidation);   
