function DataSetChartLine(nombreDeLinea, data, color) {
    this.label = nombreDeLinea,
        this.fill = false,
        this.tension = 0.1, // Antes 'lineTension', ahora 'tension'
    ...
    self.actualizarGrafico = function () {
        $.getJSON(urlGenerarCamionesDia, { MaterialId: self.materialSeleccionado }, function (data) {
            ...
            if (myLineChartDia != null) {
                myLineChartDia.destroy(); // Destruir gráfico anterior antes de crear uno nuevo
            }
            ...
            myLineChartDia = new Chart(ctxd, {
                ...
                options: {
                    animation: false,
                    plugins: {
                        legend: { // La configuración de la leyenda se mueve a 'plugins.legend'
                            display: true
                        }
                    },
                    scales: {
                        y: { // Antes 'yAxes', ahora 'y'
                            beginAtZero: true,
                            min: 0
                        },
                        x: { // Antes 'xAxes', ahora 'x'
                            beginAtZero: true,
                            min: 0
                        }
                    }
