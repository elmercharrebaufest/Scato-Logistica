...
namespace Molinos.Scato.WebMobile.Helpers
{
    public static class ExtensionesWebGrid
    {
        ...
        {
            return grid.Column("verFotos", "", f => html.Raw(html.BotonLink("", "Index", "Foto", new { f.id }, style + " ajax-popup-link", "icon-picture", true).ToHtmlString()), "editar-borrar-columna", false);
        }

        public static WebGridColumn ColumnaEjecutar(this WebGrid grid, HtmlHelper html, string style = "")
        {
            return grid.Column("ejecutar", "", f => html.Raw(html.BotonLink(Textos.Ejecutar, "Ejecutar", "ListaDeCamiones", new { f.id, proximaAccion = (string)f.ProximaAccion, codigo = f.Codigo }, style, "icon-play", true).ToHtmlString()), "editar-borrar-columna", false);
        }
        ...
        public static WebGridColumn ColumnaSeleccionarBalanzada(this WebGrid grid, HtmlHelper html, string controller, string style = "panel-darkgreen", bool useAjax = true)
        {
            return grid.Column("Seleccionar", "Ver", f => html.Raw(html.BotonLink(Textos.Seleccionar, "Seleccionar", controller,new { f.idCarga,f.NumeroBalanza}, style + ((useAjax) ? " ajax-ver-link" : string.Empty), "fa fa-list", true).ToHtmlString()),
                                                        "editar-borrar-columna", false);
        }
